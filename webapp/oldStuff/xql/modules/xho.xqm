...
declare 
    function xho:createFooter($node as node()*, $model as map(*), $foo as xs:string?) as element() {
    $foo
};

declare 
    %templates:wrap %templates:default("language", "en")
function xho:hello-world($node as node(), $model as map(*), $language as xs:string, $user as xs:string) as xs:string {
    switch($language)
        case "de" return
            "Hallo " || $user
        case "it" return
...
declare function xho:createCommonFooter($node as node(), $model as map(*)) as item()* {
    let $html_pixDir := (:wega:getOption('html_pixDir'):) 'bla'
    let $baseHref := (:wega:getOption('baseHref'):) 'myserver'
