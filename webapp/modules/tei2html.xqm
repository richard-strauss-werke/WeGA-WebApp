...
declare function tei2html:process($nodes as node()*, $options as map()*) {
    let $options := if($options) then map:new(($options, map:entry('optionsFile', document-uri($config:options-file)))) else map:new(map:entry('optionsFile', document-uri($config:options-file)))
