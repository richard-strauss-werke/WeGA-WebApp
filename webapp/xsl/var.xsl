...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:wega="http://xquery.weber-gesamtausgabe.de/webapp/functions/utilities" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:rng="http://relaxng.org/ns/structure/1.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" version="2.0">
    ...
    <xsl:preserve-space elements="tei:cell tei:p tei:hi tei:persName tei:rs tei:workName tei:characterName tei:placeName tei:code tei:eg tei:item tei:head"/>
    ...
    <xsl:template match="tei:div">
        ...
        <xsl:if test="$collapseBlock and @type = $collapseable">
            <xsl:element name="span">
                ...
                <xsl:attribute name="onclick" select="concat('$(&#34;', $uniqueID, '&#34;).toggle()')"/>
                <xsl:text>[</xsl:text>
    ...
    <xsl:template match="tei:figure" priority="1">
        ...
        <xsl:element name="img">
            <xsl:apply-templates select="@xml:id"/>
            <xsl:attribute name="src" select="concat($digilibDir, replace(tei:graphic/@url, '/db/images/', ''), '&amp;dh=', $figureHeight, '&amp;mo=q2')"/>
            <xsl:attribute name="title" select="tei:figDesc/tei:title[@xml:lang = $lang and @level = 'a']"/>
            <xsl:attribute name="alt" select="tei:figDesc/tei:title[@xml:lang = $lang and @level = 'a']"/>
    ...
    <xsl:template name="createSecNo">
        ...
        <xsl:value-of select="count($div/preceding-sibling::tei:div[ancestor-or-self::tei:div/@xml:lang=$lang]) + 1"/>
        <xsl:if test="$dot">
            <xsl:text>. </xsl:text>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="createToc">
        ...
        <xsl:element name="div">
            ...
            <xsl:element name="ul">
                <xsl:for-each select="//tei:text//tei:head[not(@type='sub') and ancestor::tei:div/@xml:lang = $lang]">
                    <xsl:element name="li">
                        <xsl:element name="a">
